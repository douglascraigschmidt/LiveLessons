 ...
 *        Phaser to continue searching a variable number of
 *        words/Threads concurrently until there's no more input
 *        to process.
 */
public class CyclicSearchWithPhaser 
     ...
     * each Thread must await on mPhaser for all the other Threads
     * to complete their processing before they all attempt to
     ...
     * requires a reconfiguration to add or remove Threads from
     * the gang of tasks.
     */
    volatile int mReconfiguration;

    /**
     * Synchronizes all Threads during the reconfiguration.
     */
    ...
    CyclicSearchWithPhaser(String[] wordsToFind,
        ...
        // Create a Phaser that controls how Threads synchronize
        // on a dynamically reconfigurable barrier.  The number of
        // Threads may vary on each iteration cycle, depending on
        // the number of Strings provided as input.
        mPhaser = new Phaser() {
                ...
                protected boolean onAdvance(int phase,
                    ...
                    if (getInput() == null || registeredParties == 0)
                        ...
                        // See if we need to reconfigure the
                        // Phaser due to changes in the size of
                        // the input List.
                        mReconfiguration = newSize - oldSize;

                        // No reconfiguration needed since there
                        // was no change to the size of the input
                        // List.
                        if (mReconfiguration == 0) 
                        	// had to add 'TaskGangBarrierTest.' because [printDebugging(string)
                        	// is not part of the ancestry of this class
                            TaskGangBarrierTest.printDebugging("@@@@@ Started cycle "
                                           + mCurrentCycle.get()
                                           + " with same # of Threads ("
                                           + newSize
                                           + ") @@@@@");

                        // A reconfiguration is needed since the size
                        // of the input List changed.
                        else {
                        	TaskGangBarrierTest.printDebugging("@@@@@ Started cycle "
                                           + mCurrentCycle.get()
                                           + " with "
                            ...
                            // Create a new CyclicBarrier to
                            // manage the reconfiguration.  We can
                            // use a CyclicBarrier here since
