...
public class ZippyQuotesTest {
    ...
    /**
     * The number of random Zippy th' Pinhead quotes to process.
     */
    ...
    public void testSubscribeAndCancel() {
        ...
        subscriptionResponse = mZippyProxy
            ...
            .cancelConfirmed(subscriptionResponse);
    ...
    public void testValidSubscribeForQuotes() {
        ...
        Flux<Quote> zippyQuotes = mZippyProxy
            ...
            // that emits Quote objects from the server.
            .getAllQuotes(subscriptionRequest)

            // Print each Zippy quote emitted by the Flux<Quote>.
            .doOnNext(m ->
                      Options.debug(TAG, "Quote: " + m.quote()))
    ...
    public void testInvalidSubscribeForAllQuotes() {
        ...
        Mono<Void> mono = mZippyProxy
            ...
            // subscriptionRequest.
            .cancelUnconfirmed(subscriptionResponse);
        ...
        Flux<Quote> zippyQuotes = mZippyProxy
            ...
            // was cancelled.
            .getAllQuotes(subscriptionResponse);
    ...
    public void testGetRandomQuotesSubscribed() {
        ...
        var randomIndicesM = mZippyProxy
        ...
        StepVerifier
            .create(zippyQuotes)
            .expectNextMatches(m -> m
                               .quoteId() == randomIndices[0])
            .expectNextMatches(m -> m
                               .quoteId() == randomIndices[1])
            .expectNextMatches(m -> m
                               .quoteId() == randomIndices[2])
            .expectNextMatches(m -> m
                               .quoteId() == randomIndices[3])
            .expectNextMatches(m -> m
                               .quoteId() == randomIndices[4])
            .verifyComplete();
