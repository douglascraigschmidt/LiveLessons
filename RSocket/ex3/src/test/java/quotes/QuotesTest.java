...
import quotes.utils.SentimentAnalyzer;
...
public class QuotesTest {
    ...
    public void testGetAllQuotesWithBackpressure() {
        ...
        Flux
            ...
            .just(makeQuoteSubscriber("Macbeth"),
                makeQuoteSubscriber("Hamlet"))
     ...
     *
     * @param play The type of quote to request (e.g., ZIPPY, HANDEY, etc.)
    ...
    (String play) {
        ...
        BlockingSubscriber<Quote> blockingSubscriber =
            new BlockingSubscriber<>
                (quote -> {
                    ...
                    Options.debug(TAG, play + ": " + quote.getQuote());
                    SentimentAnalyzer.printSentiment(quote.getQuote());
                },
        ...
        Mono<Subscription> subscriptionRequest = mQuotesProxy
            .subscribe(UUID.randomUUID(), play);
