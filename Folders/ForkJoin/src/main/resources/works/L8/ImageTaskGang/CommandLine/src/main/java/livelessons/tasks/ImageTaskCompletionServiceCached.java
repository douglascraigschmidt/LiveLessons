 ...
 * in conjunction with a cached thread pool to concurrently download a
 * list of images from web servers, apply image processing filters to
 * each image, and store the results in files that can be displayed to
        ...
        Thread t = new Thread(() -> {
            ...
            for (int i = 0; i < resultsCount; ++i)
                try {
                    ...
                    if (false)
                        System.out.println
                                ("ImageTaskCompletionServiceCached: Operations on URL "
                                        + image.getSourceURL()
                                        + "\n       in file "
                                        + image.getFileName()
                    ...
                    System.out.println("ImageTaskCompletionServiceCached: get() ExecutionException");
                } catch (InterruptedException e) {
                    System.out.println("ImageTaskCompletionServiceCached: get() InterruptedException");
                }

            System.out.println("ImageTaskCompletionServiceCached: "
                    + succeeded
