 ...
 * {@link Mono} objects into a single {@link Flux} object that is
 * triggered when all {@link Mono} objects in the stream complete.
 */
public class MonosFluxCollector<T>
     ...
     */
    @Override
    public Function<List<Mono<T>>, Flux<T>> finisher() {
        // Return a Flux of completed elements of type T.
        return monos -> Mono
            ...
            // When all monos have completed get a Flux of elements of
            // type T.
            .flatMapMany(v -> Flux
                         // Create a flux stream of completable monos.
                         .fromIterable(monos)

                         // Use map() to block() all monos and
                         // yield objects of type T (block() will
                         // not actually block).
                         .map(Mono::block));
    }
     ...
     * MonosFluxCollector}.
     *
     * @return A new {@link MonosFluxCollector}
     */
    public static <T> Collector<Mono<T>, ?, Flux<T>>
    toMonoFlux() {
        return new MonosFluxCollector<T>();
    }
}
