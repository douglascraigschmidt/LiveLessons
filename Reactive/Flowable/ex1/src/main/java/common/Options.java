...
public class Options {
     ...
     * 'M' - MISSING (onNext() events are written without any buffering or dropping.)
     * 'L' - LATEST (Downstream will get only the latest signals from upstream)
     */
    private char mOverflowStrategy = 'M';
     ...
     */
    public BackpressureStrategy overflowStrategy() {
        return switch (mOverflowStrategy) {
            case 'D' -> BackpressureStrategy.DROP;
            case 'B' -> BackpressureStrategy.BUFFER;
            case 'E' -> BackpressureStrategy.ERROR;
            case 'M' -> BackpressureStrategy.MISSING;
            case 'L' -> BackpressureStrategy.LATEST;
            default -> throw new IllegalArgumentException("invalid BackpressureStrategy");
        };
    }
